...
class SendWhatsappEventRegistration(models.TransientModel):
    _name = 'send.whatsapp.event.registration'
    _description = 'Send Whatsapp event registration'

    partner_id = fields.Many2one('res.partner', string="partner")
    default_messege_id = fields.Many2one('whatsapp.template', domain="[('category', '=', 'events')]")
    ...
    def _onchange_message(self):
        res_id = self.env['event.registration'].browse(self._context.get('active_id'))
